...
import { ArrowRight, CalendarClock, CheckCircle2, FileCog, KeyRound, Mail, Server, ShieldAlert, SlidersHorizontal, Truck, UsersRound, PlugZap } from "lucide-react";
...
export default async function AdministrationPage() {
  ...
  const integrations = await getRuntimeIntegrationModes();
  const production = env.NODE_ENV === "production";
  const gates = [
    { label: "Authentication", ready: env.AUTH_MODE !== "development", note: integrations.authenticationMode === "local" ? "Secure local sign-in active; Microsoft Entra can be enabled later" : "Microsoft Entra OpenID Connect active", icon: KeyRound },
    { label: "Email delivery", ready: integrations.emailMode === "smtp", note: integrations.emailMode === "capture" ? "Messages are captured without external delivery" : "SMTP delivery active", icon: Mail },
  ...
  return (
    <div className="page-stack">
      ...
      <section className="admin-module-grid">
        ...
        <Link className="panel admin-module-card" href="/settings/integrations"><span className="record-icon"><PlugZap size={20} /></span><div><strong>Microsoft & email integrations</strong><p>Configure, validate, and activate Microsoft Entra sign-in and SMTP delivery without a deployment.</p></div><ArrowRight size={18} /></Link>
      </section>
